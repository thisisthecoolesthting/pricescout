...
branch: feat/dual-surface-copy
dispatch_id: PRICESCOUT-DUAL-SURFACE-COPY-002
...
agent: cursor
operator_blocked_on: []
reply: cursor-dispatch/inbox/2026-04-29-002-pricescout-dual-surface-copy.reply.md
tools: ["read_file", "write_file", "edit_block", "bash_command", "git_operation"]
dangerously_allow_bash: true
---

Read **`docs/SOLUTIONSTORE_SAAS_SPINE.md`** §6 (page recipe) and §7 (visual tokens) before acting. Reference **shiftdeck.tech** live to see how the spine's six-section recipe lands in production — PriceScout should match that rhythm.

# 002 — Surface the dual-camera story across the marketing site

## Context

PriceScout's `/scan` route already supports **TWO** camera surfaces in code:

1. **Phone (primary)** — Expo / `expo-camera` in `mobile/app/(tabs)/index.tsx`. Android live, iOS in review.
2. **Web with local camera (secondary)** — `getUserMedia` + `<video>` + barcode reader in `src/components/Scanner.tsx`. Works on any laptop or POS terminal with a webcam, USB overhead cam, or built-in FaceTime camera.
...
The audience this product targets — thrift stores, estate sale crews, yard sale runners — is a phone-first crowd, but a meaningful slice has a **back-room laptop with a webcam pointed at the triage table**. That use case sells itself when surfaced. Right now it's invisible.

ShiftDeck's marketing surface mentions BOTH "manage from your phone" and "any browser, any device" — same idea. We mirror that pattern.
...
- Left: phone in hand scanning a sweater (existing `hero-phone-mockup.jpg` or refresh)
- Right: laptop with a USB cam over an item, browser tab open to `/scan` showing the same verdict UI

Generate the laptop+webcam asset via Leonardo Flux Schnell (operator's existing pipeline). Prompt:
> "back room of a thrift store, laptop on a wooden table with overhead USB webcam pointed at a vintage sweater on the counter, soft warm lighting, cream and mint accents, photorealistic, 16:9, 1024x576"

Save to `public/images/hero-laptop-webcam.jpg`. Width 880, height 587 to match the phone aspect.
...
- [ ] git mv outbox→done + proof JSON `build/proof/PRICESCOUT-DUAL-SURFACE-COPY-002.json`

## Out of scope

- New `/scan?mode=kiosk` route or device-pairing UI (those are dispatches 005 and 006 — separate work)
- A11y rewrite of the whole site (Codex dispatch 010)
- Stripe device-addon Price ID wiring (operator-blocked on Price IDs from Stripe Dashboard)
- iOS app store copy (Apple Dev approval pending)
- Translating any of this to es / pt-BR (Codex dispatch 011)
